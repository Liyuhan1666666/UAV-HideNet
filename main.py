...
import methods.uav_hidenet


def parse_config():
    parser = argparse.ArgumentParser("Training and evaluation script")
    parser.add_argument("--config", default="./configs/uav_hidenet/cod_uav_hidenet.py", type=str)
    ...
    config.test_only = args.test_only  # 传给config
    ...
    for batch_id, batch in pgr_bar:
        ...
        if tta_setting.enable:
            logits = pipeline.test_aug(
                model=model, data=batch_images, strategy=tta_setting.strategy, reduction=tta_setting.reduction
            )
        else:
...
def training(model, cfg) -> pipeline.ModelEma:
    ...
    for curr_epoch in range(start_epoch, cfg.train.num_epochs):
        ...
        for batch_idx, batch in enumerate(tr_loader):
            scheduler.step(curr_idx=curr_iter)
...
def main():
    ...
    # Test-only runs also create logs, metric files, and prediction folders.
    # Initialize them unless we are continuing an existing experiment.
    ...
    misc.initialize_seed_cudnn(seed=cfg.base_seed, deterministic=cfg.deterministic)
    ...
    if cfg.test_only:
        # 纯测试：只加载权重，直接测试，不训练
        print(">>>> Test only mode, loading weights and testing...")
